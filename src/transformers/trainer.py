                ...
                if self.args.fsdp_min_num_params > 0:
                    ...
                    if isinstance(self.args.fsdp_transformer_layer_cls_to_wrap, str):
                        transformer_cls_to_wrap = {
                            get_module_class_from_name(model, self.args.fsdp_transformer_layer_cls_to_wrap)
                        }
                    elif isinstance(self.args.fsdp_transformer_layer_cls_to_wrap, list) and all(
                        isinstance(elem, str) for elem in self.args.fsdp_transformer_layer_cls_to_wrap
                    ):
                        transformer_cls_to_wrap = {
                            get_module_class_from_name(model, name)
                            for name in self.args.fsdp_transformer_layer_cls_to_wrap
                        }
                    else:
                        raise Exception(
                            "fsdp_transformer_layer_cls_to_wrap must be either a string or a list of strings."
                        )
